<div class="p-6">
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-title-md2 font-bold text-black dark:text-white">Daftar Surat Masuk</h2>
            <p class="text-sm font-medium text-gray-500">Kelola dan lihat daftar surat masuk yang telah terdaftar.</p>
        </div>

        <!-- Right: Actions -->
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            <!-- Add button -->
            @can('create Surat Masuk')
    ...
    <div class="mb-4 flex flex-col md:flex-row gap-4 justify-between items-center">
        <div class="w-full md:w-1/3 relative">
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nomor, pengirim, perihal..." class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-2.5 pl-10 font-normal text-gray-700 outline-none transition focus:border-blue-600 active:border-blue-600 dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-blue-600" />
            ...
            <table class="w-full table-auto">
                <thead>
                    <tr class="bg-gray-50 text-left dark:bg-gray-900">
                        ...
                        <th class="py-4 px-4 font-medium text-gray-900 dark:text-white cursor-pointer" wire:click="sortBy('pengirim')">Pengirim</th>
                ...
                <tbody>
                    ...
                    <tr class="border-t border-gray-200 dark:border-gray-800">
                        ...
                        <td class="py-4 px-4 text-sm">
                            <p class="text-gray-800 dark:text-gray-300">{{ $surat->pengirim }}</p>
                        </td>
                        <td class="py-4 px-4 text-sm">
                        ...
                        <td class="py-4 px-4 text-sm">
                            <span class="inline-flex rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-500/15 dark:text-blue-500">
                                {{ $surat->status }}
                            </span>
                        </td>
                        <td class="py-4 px-4 text-right xl:pr-6">
                            <div class="flex items-center justify-end space-x-3.5">
                                ...
                                <button wire:click="view({{ $surat->id_surat_masuk }})" class="hover:text-green-600 dark:text-gray-400 dark:hover:text-white" title="Lihat Detail">
                                ...
                                <button wire:click="edit({{ $surat->id_surat_masuk }})" class="hover:text-blue-600 dark:text-gray-400 dark:hover:text-white" title="Edit">
                                    <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                ...
                                @endcan
                                @can('delete Surat Masuk')
                                <button wire:click="deleteConfirm({{ $surat->id_surat_masuk }})" class="hover:text-red-600 dark:text-gray-400 dark:hover:text-white">
                                    <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <x-dialog-modal wire:model.live="isOpen" maxWidth="4xl">
        <x-slot name="title">
            {{ $suratId ? 'Edit Surat Masuk' : 'Tambah Surat Masuk' }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <x-label for="nomor_agenda" value="Nomor Agenda" />
                    <input id="nomor_agenda" type="text" class="mt-1 block w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-gray-700 outline-none transition focus:border-blue-600 active:border-blue-600 dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-blue-600" wire:model="nomor_agenda" />
                    <x-input-error for="nomor_agenda" class="mt-2" />
                </div>
                <div>
                ...
                <div>
                    <x-label for="tanggal_terima" value="Tanggal Terima *" />
                    <input id="tanggal_terima" type="date" class="mt-1 block w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-gray-700 outline-none transition focus:border-blue-600 active:border-blue-600 dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-blue-600" wire:model="tanggal_terima" />
                    <x-input-error for="tanggal_terima" class="mt-2" />
                </div>
                <div>
                    <x-label for="pengirim" value="Pengirim *" />
                    <input id="pengirim" type="text" class="mt-1 block w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-gray-700 outline-none transition focus:border-blue-600 active:border-blue-600 dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-blue-600" wire:model="pengirim" />
                    <x-input-error for="pengirim" class="mt-2" />
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    @endif
                    @if ($suratId)
                        @php $existingLampiran = \App\Models\SuratMasuk::find($suratId)->lampiran; @endphp
    ...
    <x-confirmation-modal wire:model.live="isConfirmDeleteOpen">
        ...
        <x-slot name="content">
            Apakah Anda yakin ingin menghapus surat masuk ini beserta lampirannya?
        </x-slot>
    ...
    <x-dialog-modal wire:model.live="isViewModalOpen" maxWidth="4xl">
        ...
        <x-slot name="content">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <h4 class="text-sm font-semibold text-gray-500 mb-1">Nomor Agenda</h4>
                    <p class="text-gray-900 dark:text-white">{{ $viewData->nomor_agenda ?? '-' }}</p>
                </div>
                <div>
                ...
                <div>
                    <h4 class="text-sm font-semibold text-gray-500 mb-1">Tanggal Terima</h4>
                    <p class="text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($viewData->tanggal_terima)->format('d F Y') }}</p>
                </div>
                <div>
                    <h4 class="text-sm font-semibold text-gray-500 mb-1">Pengirim</h4>
                    <p class="text-gray-900 dark:text-white">{{ $viewData->pengirim }}</p>
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-gray-900 dark:text-white">{{ $viewData->ringkasan ?? '-' }}</p>
                </div>
                <div class="md:col-span-2">
